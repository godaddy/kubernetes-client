...
function mapCertificateData(source, certKey, mapFn) {
  return Object.keys(source).reduce((dest, key) => {
    if (key === certKey) {
      dest[`${key}-data`] = mapFn(source[key]);
    } else {
      dest[key] = source[key];
    }
...
function readCertificates(cfgPath, config) {
  const readCert = readCertFn(cfgPath);

  const mapping = [
    ['clusters.cluster', ['certificate-authority']],
    ['users.user', ['client-certificate', 'client-key']]
  ];

  mapping.forEach((params) => {
    const keys = params[0].split('.', 2);
    const topLevel = keys[0], subLevel = keys[1], certKeys = params[1];

    certKeys.forEach((certKey) => {
      config[topLevel].forEach((item) => {
        item[subLevel] = mapCertificateData(item[subLevel], certKey, readCert);
      });
    });
  });
...
function loadKubeconfig(cfgPath) {
  ...
  return readCertificates(cfgPath, config);
}
