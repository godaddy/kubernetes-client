      ...
      if (authProvider) {
        ...
        if (now < expires) {
          ...
          if (config['cmd-path']) {
            ...
            type = 'openid';
          } else if (config['access-token']) {
            type = 'skip';
            auth.bearer = config['access-token'];
          } else {
            return reject('Failed to determine auth-provider type, or isn\'t supported');
          }

          if (type !== 'skip') {
            return refreshAuth(type, config)
              .then(response => {
                return resolv({
                  url: cluster.server,
                  auth: Object.keys(response).length ? response : null,
                  ca: ca,
