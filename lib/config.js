...
function normalizePathFn(cfgPath) {
  const configDir = path.dirname(cfgPath);
  return (subPath) => path.isAbsolute(subPath) ? subPath : path.join(configDir, path.normalize(subPath));
}

function mapSpecificKeysFn(mapFn, srcKey) {
  return (source) => {
    return Object.keys(source).reduce((dest, key) => {
      dest[key] = (key === srcKey) ? mapFn(source[key]) : source[key];
      return dest;
    }, {});
  };
}
...
function mapCertificates(cfgPath, config) {
  const normalizePath = normalizePathFn(cfgPath);
  const compose = (f, g) => (x) => f(g(x));
  const mapArrayFn = (fn) => (items) => items.map(fn);

  const mapCertFn = (srcKey) => mapSpecificKeysFn(normalizePath, srcKey);
