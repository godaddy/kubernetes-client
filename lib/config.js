...
function readCertFn(cfgPath) {
  const configDir = path.dirname(cfgPath);
  const normalizePath = (subPath) => path.join(configDir, path.normalize(subPath));

  return (file) => Buffer.from(fs.readFileSync(normalizePath(file))).toString('base64');
}

function mapSpecificKeysFn(mapFn, srcKey, dstKey = srcKey) {
  return (source) => {
    return Object.entries(source).reduce((dest, [key, value]) => {
      if (key === srcKey) {
        dest[dstKey] = mapFn(value);
      } else {
        dest[key] = value;
      }
...
function mapCertificates(cfgPath, config) {
  const readCert = readCertFn(cfgPath);
  const compose = (f, g) => (x) => f(g(x));
  const mapArrayFn = (fn) => (items) => items.map(fn);

  const mapCertFn = (srcKey) => mapSpecificKeysFn(readCert, srcKey, `${srcKey}-data`);

  const mapCluster = mapSpecificKeysFn(mapCertFn('certificate-authority'), 'cluster');
  const mapUser = mapSpecificKeysFn(compose(mapCertFn('client-certificate'), mapCertFn('client-key')), 'user');

  const mapClusters = mapSpecificKeysFn(mapArrayFn(mapCluster), 'clusters');
  const mapUsers = mapSpecificKeysFn(mapArrayFn(mapUser), 'users');

  return compose(mapClusters, mapUsers)(config);
}
